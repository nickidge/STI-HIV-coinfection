...
strat_interp = function(strat0){
  nvars = ncol(strat0) - 1
  st_interp = array(0, dim = c(length(tvec_base), nvars))
  for (i_base in 1:nvars){
    st_interp[,i_base] = approx(unlist(strat0[,1]), unlist(strat0[,(i_base+1)]), xout=tvec_base, rule=2)$y
  }
  rownames(st_interp) = tvec_base
  return(st_interp)
}
...
r_diag_sti_interp = array(0, dim = c(length(tvec_base), 3))
for (i_base in 1:3){
  r_diag_sti_interp[,i_base] = approx(unlist(r_diag_sti0[,1]), unlist(r_diag_sti0[,(i_base+1)]), xout=tvec_base, rule=2)$y
}
rownames(r_diag_sti_interp) = tvec_base
...
condom_by_HIV_interp = array(0, dim = c(length(tvec_base), 3))
for (i_base in 1:3){
  condom_by_HIV_interp[,i_base] = approx(unlist(condom_by_HIV0[,1]), unlist(condom_by_HIV0[,(i_base+1)]), xout=tvec_base, rule=2)$y
}
rownames(condom_by_HIV_interp) = tvec_base

# proportion of people on PrEP
prop_prep = base_raw[,c(1,2)]
prop_prep = prop_prep[!is.na(prop_prep[,2]),]
prop_prep_interp = approx(unlist(prop_prep[,1]), unlist(prop_prep[,2]), xout=tvec_base, rule=2)$y
names(prop_prep_interp) = tvec_base
...
r_diag_HIV_interp = approx(unlist(r_diag_HIV_scenario[,1]), unlist(r_diag_HIV_scenario[,2]), xout=tvec_base, rule=2)$y
names(r_diag_HIV_interp) = tvec_base
...
prop_testing_interp = array(0, dim = c(length(tvec_base), 3))
for (i_base in 1:3){
  prop_testing_interp[,i_base] = approx(unlist(prop_testing0[,1]), unlist(prop_testing0[,(i_base+1)]), xout=tvec_base, rule=2)$y
}
rownames(prop_testing_interp) = tvec_base
...
t_testing_interp = array(0, dim = c(length(tvec_base), 3, 2))
for (j_base in 1:2){
  for (i_base in 1:3){
    t_testing_interp[,i_base,j_base] = approx(unlist(t_testing0[,1]), unlist(t_testing0[,(i_base+3*j_base-2)]), xout=tvec_base, rule=2)$y
  }
}
rownames(t_testing_interp) = tvec_base
