...
fig_4_left = ggplot(data = subset(fig_4_sti_df, sti_group=="incidence_sti" & (HIV_group=="HIV_minus" | HIV_group=="HIV_plus")), aes(x=year, y=value, colour=HIV_group, linetype=scenario)) +
  geom_line(lwd=1.3) +
  labs(x=NULL,y="Estimated new syhilis infections in year",
       title="Estimated syphilis incidence: Victorian GBM by HIV status", subtitle="PrEP scenario",
  ...
  coord_cartesian(xlim=c(2009.5,2025.5),
                  ylim=c(0,2000))+#1.1*fig_4_maxval_left)) +
...
fig_4_right = ggplot(data = subset(fig_4_prev_df, (prev_group=="HIV-" | prev_group=="HIV+")), aes(x=year, y=value, colour=prev_group, linetype=scenario)) +
  ...
  labs(x=NULL, y="Prevalence",
       title="Estimated STI prevalence among Victorian GBM by HIV status", subtitle="PrEP scenario",
       colour=NULL, linetype=NULL) +
  theme(legend.box = 'horizontal',
...
ggsave("fig_4.pdf", plot=fig_4, width=130, height=100, units="mm")
ggsave("fig_4.png", plot=fig_4, width=130, height=100, units="mm", dpi=500)
# browseURL("fig_4.pdf")
# browseURL("fig_4.png")
