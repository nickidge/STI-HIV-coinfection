...
heat_mat = matrix(0, nrow=gel_up_res*gel_down_res*eff_gel_l, ncol=4, dimnames=list(NULL, c("gel_up", "gel_down", "eff_gel", "value")))
...
for(i_heat in 1:(gel_up_res*gel_down_res*eff_gel_l)){
  gel_up_heat = gel_up_seq[i_heat]
  gel_down_heat = gel_down_seq[i_heat]
  eff_gel_heat = eff_gel_seq[i_heat]
...
heat_df$eff_gel_label<-paste("Gel eff. =", percent(heat_df$eff_gel))
...
heat_p = heat_p +
  ...
  labs(fill="% change from base\ncumulative incidence",
    ...
    title=paste0("Change in cumulative incidence of Gonorrhoea between\n", split_year, " and ", heat_year, " due to introduction of gel condoms"),
